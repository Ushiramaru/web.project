...
public class ConferenceBlockCommand extends AbstractCommand {
    ...
    public ConferenceBlockCommand(RequestParameterExtractor parameterExtractor,
    ...
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) throws ServiceException {
        ...
        Long blockConferenceId = extractor.extractLong(request, CONFERENCE_ID_PARAMETER_NAME);
        if (!validator.isValid(CONFERENCE_ID_PARAMETER_NAME, blockConferenceId)) {
            throw new ServiceException(INVALID_PARAMETER_MESSAGE.replace("?", CONFERENCE_ID_PARAMETER_NAME));
        }
        conferenceService.blockById(blockConferenceId);
